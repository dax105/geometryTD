...
public class Enemy extends TickListener {
	...
	public Enemy(Game game, Point[] points, int life, boolean f) {
		super(game);
	...
	protected void renderTick(float ptt) {
		if (!dead) {
			...
			if (img == 6) {
			GLUtil.drawAtlasTexture(super.game.getTextureManager(), 1, img, cx-16, cx+16, cy-16, cy+16);
			} else {
				GLUtil.drawAtlasTexture(super.game.getTextureManager(), 1, img, cx-64, cx+64, cy-64, cy+64);
			}
		}	
	}
