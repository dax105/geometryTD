...
public abstract class Tower extends TickListener {
	...
	public Tower(Game game, int textureID, Level l) {
		...
		this.rec.setSize(
				(int) this.game.getTextureManager().getTexture(1).SheetSize.x,
	...
	protected void tick() {
		...
		if (timer <= 0) {
			...
			Point pos = new Point(this.rec.x + World.TOWER_WIDTH_HALF,
					this.rec.y + World.TOWER_WIDTH_HALF);
			...
			if (closest != null) {
				...
				this.l.shoot(new ShootEffect(org.newdawn.slick.Color.yellow,
						pos, new Point((int) closest.x, (int) closest.y), 5, 2));
	...
	protected void renderTick(float ptt) {
		GL11.glTranslatef(+this.rec.x + World.TOWER_WIDTH_HALF, +this.rec.y
				+ World.TOWER_WIDTH_HALF, 0);
		GL11.glPushMatrix();
		GL11.glRotatef((float) (System.nanoTime() / 50000000D), 0, 0, 1);
		GL11.glTranslatef(-this.rec.x - World.TOWER_WIDTH_HALF, -this.rec.y
				- World.TOWER_WIDTH_HALF, 0);

		GLUtil.drawAtlasTexture(super.game.getTextureManager(), 1, this.tID,
				this.rec.x, this.rec.y);
		GL11.glPopMatrix();
		GL11.glTranslatef(-this.rec.x - World.TOWER_WIDTH_HALF, -this.rec.y
				- World.TOWER_WIDTH_HALF, 0);
	}
